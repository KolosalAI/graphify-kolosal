...
from python.graphify.extract import extract_python, extract, collect_files, _make_id, extract_bash, extract_json, _DISPATCH
...
def test_collect_files_from_dir():
    from python.graphify.extract import _DISPATCH
...
def _legacy_collect_files(target, *, root=None):
    ...
    from python.graphify.detect import _is_ignored, _is_noise_dir, _load_graphifyignore
...
def test_extract_generic_surfaces_tree_sitter_version_mismatch_hint(monkeypatch):
    ...
    import sys
    import types
    from python.graphify.extract import _extract_generic, LanguageConfig
...
def test_extract_js_destructured_require_imports_from():
    ...
    from python.graphify.extract import extract_js
...
def test_extract_js_destructured_require_named_symbols():
    ...
    from python.graphify.extract import extract_js, _make_id, _file_stem
    result = extract_js(FIXTURES / "cjs_require.js")
...
def test_extract_tsx_uses_tsx_grammar():
    ...
    from python.graphify.extract import _TSX_CONFIG, _TS_CONFIG
...
def test_extract_falls_back_to_sequential_when_parallel_returns_false(tmp_path, monkeypatch):
    ...
    from python.graphify import extract as extract_mod
...
def test_extract_bash_skip_builtins_in_calls():
    from python.graphify.extract import _file_stem, _make_id
...
def test_extract_bash_via_dispatch():
    from python.graphify.extract import _get_extractor
...
def test_semantic_reference_edges_carry_context_and_source():
    from python.graphify.extract import _semantic_reference_edge
...
def test_dart_child_node_ids_are_stem_based(tmp_path):
    ...
    from python.graphify.extract import extract_dart, _file_stem, _make_id
...
def test_non_colliding_path_id_is_not_salted(tmp_path):
    ...
    from python.graphify.extractors.base import _file_stem
    from python.graphify.ids import make_id
