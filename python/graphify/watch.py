...
from python.graphify.paths import GRAPHIFY_OUT as _GRAPHIFY_OUT
...
from python.graphify.detect import (
    CODE_EXTENSIONS,
    DOC_EXTENSIONS,
    PAPER_EXTENSIONS,
    IMAGE_EXTENSIONS,
...
def _check_shrink(
    ...
    *,
    ...
    if rebuilt_sources is not None:
        from python.graphify.build import _norm_source_file
...
def _rebuild_code(
    ...
    *,
    ...
    try:
        from python.graphify.extract import extract, _get_extractor
        from python.graphify.detect import detect
        from python.graphify.build import build_from_json, _norm_source_file as _nsf
        from python.graphify.cluster import cluster, remap_communities_to_previous, score_all
        from python.graphify.analyze import god_nodes, surprising_connections, suggest_questions
        from python.graphify.report import generate
        from python.graphify.export import to_json, to_html
        from python.graphify.security import check_graph_file_size_cap
        ...
        if no_cluster:
            ...
            from python.graphify.build import dedupe_edges as _dedupe_edges, dedupe_nodes as _dedupe_nodes
            ...
            try:
                from python.graphify.detect import save_manifest
        ...
        if missing:
            ...
            from python.graphify.cluster import label_communities_by_hub
            labels.update(label_communities_by_hub(G, missing))
        questions = suggest_questions(G, communities, labels)
        from python.graphify.report import load_learning_for_report as _llfr
        ...
        if no_change:
            ...
            if not _check_shrink(
                ...
                return False
            from python.graphify.export import backup_if_protected as _backup
            _backup(out)
        ...
        if callflow_files and not no_change:
            try:
                from python.graphify.callflow_html import write_callflow_html
